...
# Download the archive in required location and rename to jasperserver.zip. 

RUN wget \
   "https://excellmedia.dl.sourceforge.net/project/jasperserver/JasperServer/JasperReports%20Server%20Community%20Edition%206.3.0/jasperreports-server-cp-6.3.0-bin.zip" \
   -O /tmp/jasperreports-server-cp-6.3.0-bin.zip && \
   mv /tmp/jasperreports-server*zip /tmp/jasperserver.zip
